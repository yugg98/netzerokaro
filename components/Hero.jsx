    ...
    <div  className=' w-full sm:h-[80vh] h-full flex sm:flex-row flex-col justify-center items-center border-2 border-green border-spacing-4 border-dashed linearbggreen sm:px-20'>
      <motion.div variants={slideIn('left', 'tween', 0.2, 1)} initial="hidden"  whileInView="show" className="sm:w-[50%] w-full flex justify-center mt-10">
        ...
        <motion.div variants={textVariant(0.8)} className='text-black text-center sm:text-left sm:text-6xl text-4xl font-[800]'>
          ...
          <motion.div variants={textVariant(0.8)} className='text-black text-center sm:text-left sm:text-6xl text-4xl font-[800]'>{Title}</motion.div>
          
          <motion.p variants={textVariant(0.6)} className="text-xl leading-6 mt-4 text-center sm:text-left">{Parser().parse(Desc)}</motion.p>
